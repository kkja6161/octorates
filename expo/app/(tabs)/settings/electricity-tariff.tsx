...
import { Calendar, Zap, DollarSign, ChevronDown, ChevronUp } from 'lucide-react-native';
...
import { fetchStandingCharge, fetchEnergyRates, processRates } from '@/services/energyApi';
...
export default function ElectricityTariffScreen() {
  const {
  ...
  useEffect(() => {
    const loadTariffDetails = async () => {
      const details: TariffWithDetails[] = electricityAgreements.map(agreement => ({
        ...agreement,
        ratePeriods: [],
        isLoading: true,
        isExpanded: false,
      ...
      for (let i = 0; i < electricityAgreements.length; i++) {
        const agreement = electricityAgreements[i];
        ...
        try {
          ...
          const isAgile = agreement.productCode.includes('AGILE');
          const isTracker = agreement.productCode.includes('TRACKER') || agreement.productCode.includes('SILVER');
    ...
    if (electricityAgreements.length > 0) {
      loadTariffDetails();
    }
  }, [electricityAgreements, selectedRegion]);
                            ...
                            <View style={styles.detailRow}>
                              <View style={styles.detailLabel}>
                                ...
                                <Text style={styles.detailLabelText}>
                                  {tariff.productCode.includes('AGILE') || tariff.productCode.includes('TRACKER') || tariff.productCode.includes('SILVER') ? 'Average rate' : 'Rate'}
                                </Text>
                              </View>
        ...
        {electricityAgreements.length === 0 && (
          <View style={styles.emptyState}>
            <Text style={styles.emptyStateText}>
              No tariff history found. Please connect your Octopus account first.
            </Text>
          </View>
        )}
      </ScrollView>
